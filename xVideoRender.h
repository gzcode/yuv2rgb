...
public:
	VideoRender(HWND hWnd, LONG nWidth, LONG nHeight, std::uint32_t dwFourCC)
	{
		...
		for (int x = 0; x < 1; ++x)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					if (dwFourCC == MAKEFOURCC('N', 'V', '1', '2') || dwFourCC == MAKEFOURCC('I', '4', '2', '0') || dwFourCC == MAKEFOURCC('J', '4', '2', '0'))
					{
					...
					{
						//memset(d3d_rect.pBits, 0, d3d_rect.Pitch);
					}
		...
		D3DXCreateLine(m_pDirect3DDevice, &m_pDirect3DLine);
	...
	int SnapShot(std::string pic, RECT * rect)
	{
		if (!m_pDirect3DDevice)
		{
			return -1;
		...
		if (m_pDirect3DSurfaceRender)
		{
			/*HRESULT hr = D3DXSaveSurfaceToFileA(pic.c_str(), _D3DXIMAGE_FILEFORMAT::D3DXIFF_JPG, m_pDirect3DSurfaceRender, NULL, rect);
			if (SUCCEEDED(hr))
			{
				return 0;
			}*/
	...
	int Render(BYTE * pData[], int nLineSize[])
	{
		...
		m_pDirect3DDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
		m_pDirect3DDevice->StretchRect(m_pDirect3DSurfaceRender, NULL, pBackBuffer, NULL, D3DTEXF_POINT);

		//HRESULT hr = D3DXSaveSurfaceToFileA("my.jpg", _D3DXIMAGE_FILEFORMAT::D3DXIFF_JPG, pBackBuffer, NULL, NULL);
		...
		if (m_pDirect3DLine)
		{
			...
			m_pDirect3DLine->SetWidth(3.f);
			m_pDirect3DLine->SetAntialias(TRUE);
			m_pDirect3DLine->Draw(&v[0], sizeof(v) / sizeof(D3DXVECTOR2), 0xffffffff);
		}
	...
	int RenderVideoAndText(BYTE * pData[], int nLineSize[], std::string text = "", int text_x = 10, int text_y = 10, std::uint32_t text_color = D3DCOLOR_ARGB(140, 255, 255, 255), std::uint32_t format = DT_CENTER | DT_VCENTER/*DT_TOP | DT_LEFT*/)
	{
		RECT rc = { 0 };
...
private:
	...
	int FillFrame(BYTE * pData[], int nLineSize[])
	{
		...
		if (m_dwFourCC == D3DFMT_X8R8G8B8)
		{
			BYTE * ps = pData[0];
			BYTE * pd = (BYTE*)d3d_rect.pBits;
			for (int h = 0; h < m_nHeight; ++h)
			{
				int n = d3d_rect.Pitch < nLineSize[0] ? d3d_rect.Pitch : nLineSize[0];
				if (n > 0)
				{
					memcpy(pd, ps, n);
				}
...
private:
	...
	ID3DXLine *				m_pDirect3DLine = NULL;
};
